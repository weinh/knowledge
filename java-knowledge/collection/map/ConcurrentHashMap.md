...
## Map接口方法实现
### 获取元素数量
map结构hash表+链的实现，有可能节点数比表数组长度还大，也可能超出int的最大值，这里做了一个保护，虽然给出了一个错误的值，但尽量可用，1.8以后可以使用`public long mappingCount()`获取准确值
```java
    /**
     * {@inheritDoc}
     */
...
```java
    ...
    public V get(Object key) {
        ...
        if ((tab = table) != null && (n = tab.length) > 0 &&
            ...
            /*
                第一个节点的hash值为负数说明是特殊节点那么使用各自的方式查询
                节点类型有ForwardingNode，TreeBin，ReservationNode
            */
...
```java
    ...
    public boolean containsValue(Object value) {
        ...
        if ((t = table) != null) {
            ...
            // 遍历所有hash表的节点，因为没有加锁该结果有可能不是最新的
...
```java
    ...
    final V putVal(K key, V value, boolean onlyIfAbsent) {
        ...
        for (Node<K,V>[] tab = table;;) {
            ...
            if (tab == null || (n = tab.length) == 0)// 没有初始化初始化以下
                tab = initTable();
                // 里面有自旋锁内容，因为初始化不能并发处理，又不能切换线程，通过Thread.yield();让出CPU，只是为了自旋
